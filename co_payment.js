...
window.addEventListener("load", function() {
  ...
  //    write the field values from that array to the orderForm
...
function validateName() {
  // variable that targets the name field of the paymenty form
...
function luhn(idNum) {
  ...
  // retrieve the odd numbers ** the last index is always length - 1** [var i]
  for (var i = idNum.length - 1; i >= 0; i -= 2) {
    string1 += idNum.charAt
  }
  // retrieve the even number digits and double them [ var j]
  for (var i = 0; i < idNum.length - 2; i -= 2) {
    string2 += 2 * idNum.charAt(j);
  }
  // return bool for whether the sum of the digits is divisable[%] by 10
